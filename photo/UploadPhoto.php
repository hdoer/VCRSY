...
class UploadPhoto {
	...
	public function testFormat(){
		if (array_search($this->type,$this->format)=="")
		return false;
		return true;
	}
	/*保存电影*/
	public function savePhoto($iduser,$dir){
		if (!is_dir($dir))
		{
			mkdir($dir);
		}
	...
	public function insertVideo($iduser,$address){
		$length = $this->size;
		$title = $this->name;
		$result = $this->db->query("call insertPhotos('$iduser','$title',NULL,NULL,'$address','$length',1,NULL);");
		if($result){
			$this->db->close();
			return true;
		}
